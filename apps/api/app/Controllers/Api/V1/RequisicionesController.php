...
final class RequisicionesController extends BaseController
{
    public function create(): ResponseInterface
    {
        ...
        if (! $this->validateData($datos, [
            'unidad_destino_id'     => 'required|is_natural_no_zero',
            'origen'                => 'required|in_list[Compra,Yonke]',
            'descripcion_pieza'     => 'required|string|max_length[180]',
            ...
            $mensaje = isset($errores['unidad_destino_id'])
                ? 'Selecciona el tracto destino.'
                : (isset($errores['descripcion_pieza']) ? 'Describe la pieza solicitada.' : 'Datos de la requisición inválidos.');
    ...
    {
        try {
            $ruta = (new RequisicionService())->rutaFotoAutorizada($id, ActorActual::usuario());
