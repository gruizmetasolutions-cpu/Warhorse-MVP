...
final class TallerService
{
    ...
    public function registrarIngreso(array $datos, array $actor): array
    {
        ...
        // Validate that fecha_ingreso is not in the future (compared to current local date)
        $fechaIngresoStr = (string) ($datos['fecha_ingreso'] ?? '');
        if ($fechaIngresoStr !== '') {
            $fechaIngreso = new \DateTime($fechaIngresoStr);
            $hoy = new \DateTime('today');
            if ($fechaIngreso > $hoy) {
                throw new ValidacionException('La fecha de ingreso no puede ser mayor a la fecha actual.', [
                    'fecha_ingreso' => ['La fecha de ingreso no puede ser mayor a la fecha actual.'],
                ]);
            }
        }
