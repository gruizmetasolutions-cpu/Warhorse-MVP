...
export function TourOverlay() {
  ...
  if (r) {
    spotStyle = {
      ...
      borderRadius: 14, boxShadow: '0 0 0 9999px rgba(20,24,29,0.66)', border: '2px solid #F2620F',
    ...
    cardStyle = {
      position: 'fixed', left, top, width: 'min(360px,calc(100vw - 32px))', background: '#F3EFE7',
    ...
    cardStyle = {
      ...
      width: 'min(400px,calc(100vw - 32px))', background: '#F3EFE7', borderRadius: 14, padding: 22,
  ...
  return (
    <>
      ...
      <div style={cardStyle} role="dialog" aria-label={`Paso ${tourStep + 1} de ${pasosTour.length}: ${st.title}`}>
        <div style={{ height: 5, background: 'repeating-linear-gradient(135deg,#F2620F 0 12px,#16191E 12px 24px)', borderRadius: '14px 14px 0 0', margin: '-22px -22px 14px' }} />
        <div style={{ fontFamily: FD, fontWeight: 600, fontSize: 13, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#F2620F' }}>
          Paso {tourStep + 1} de {pasosTour.length}
        </div>
        <div style={{ fontFamily: FD, fontWeight: 700, fontSize: 26, lineHeight: 1.05, textTransform: 'uppercase', color: '#16191E', margin: '4px 0 8px' }}>
          {st.title}
        </div>
        <p style={{ margin: '0 0 16px', fontSize: 14.5, lineHeight: 1.55, color: '#4A4438' }}>{st.body}</p>
          ...
          {pasosTour.map((_, i) => (
            <span
              ...
              style={{
                ...
                background: i === tourStep ? '#F2620F' : i < tourStep ? '#C9C2B2' : '#EAE6DC',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <button
            ...
            style={{ background: 'none', border: 'none', color: '#8A8374', fontSize: 13.5, fontWeight: 600, cursor: 'pointer', padding: '8px 4px' }}
          >
            Saltar tutorial
          </button>
          <span style={{ flex: 1 }} />
          {tourStep > 0 && (
            <button
              ...
              style={{ background: '#fff', border: '1px solid #D8D2C4', color: '#16191E', padding: '10px 16px', borderRadius: 8, fontSize: 13.5, fontWeight: 700, cursor: 'pointer' }}
            >
              ← Atrás
            </button>
          )}
          <button
            ...
            style={{ background: '#F2620F', border: 'none', color: '#fff', padding: '10px 20px', borderRadius: 8, fontFamily: FD, fontSize: 16, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', cursor: 'pointer' }}
          >
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p style={{ margin: '0 0 6px', fontSize: 15, lineHeight: 1.55, color: '#4A4438' }}>
          Vas a cambiar el estado de la pieza <strong style={{ color: '#16191E' }}>{confirmar.pieza}</strong> del
          tracto <strong style={{ color: '#F2620F' }}>{confirmar.destino}</strong> a{' '}
          <strong style={{ color: '#2C7A44' }}>Instalado</strong>.
        </p>
        <p style={{ margin: '0 0 20px', fontSize: 15, fontWeight: 600, color: '#16191E' }}>¿Es correcto?</p>
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          ...
          <button
            onClick={ok}
            className="hv-verde"
            style={{ padding: '10px 20px', background: '#2C7A44', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 700, color: '#fff', cursor: 'pointer' }}
          >
            Sí, marcar instalada
          </button>
        </div>
